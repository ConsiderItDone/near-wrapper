...
import { ViewContractCode } from "./";
import * as Types from "..";

export function serializeViewContractCode(type: ViewContractCode): ArrayBuffer {
  const sizerContext: Context = new Context("Serializing (sizing) object-type: ViewContractCode");
  const sizer = new WriteSizer(sizerContext);
  writeViewContractCode(sizer, type);
  const buffer = new ArrayBuffer(sizer.length);
  const encoderContext: Context = new Context("Serializing (encoding) object-type: ViewContractCode");
  const encoder = new WriteEncoder(buffer, sizer, encoderContext);
  writeViewContractCode(encoder, type);
  return buffer;
}

export function writeViewContractCode(writer: Write, type: ViewContractCode): void {
...
export function deserializeViewContractCode(buffer: ArrayBuffer): ViewContractCode {
  const context: Context = new Context("Deserializing object-type ViewContractCode");
  const reader = new ReadDecoder(buffer, context);
  return readViewContractCode(reader);
}

export function readViewContractCode(reader: Read): ViewContractCode {
