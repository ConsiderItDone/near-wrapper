...
export function serializeChunk(type: Chunk): ArrayBuffer {
  const sizerContext: Context = new Context("Serializing (sizing) object-type: Chunk");
  const sizer = new WriteSizer(sizerContext);
  writeChunk(sizer, type);
  const buffer = new ArrayBuffer(sizer.length);
  const encoderContext: Context = new Context("Serializing (encoding) object-type: Chunk");
  const encoder = new WriteEncoder(buffer, sizer, encoderContext);
  writeChunk(encoder, type);
  return buffer;
}

export function writeChunk(writer: Write, type: Chunk): void {
  writer.writeMapLength(18);
  ...
  writer.context().pop();
  writer.context().push("gas_used", "BigInt", "writing property");
  writer.writeString("gas_used");
  writer.writeBigInt(type.gas_used);
  writer.context().pop();
...
export function deserializeChunk(buffer: ArrayBuffer): Chunk {
  const context: Context = new Context("Deserializing object-type Chunk");
  const reader = new ReadDecoder(buffer, context);
  return readChunk(reader);
}

export function readChunk(reader: Read): Chunk {
  ...
  let _gas_used: BigInt = BigInt.fromUInt16(0);
  ...
  if (!_gas_usedSet) {
    throw new Error(reader.context().printWithContext("Missing required property: 'gas_used: BigInt'"));
  }
  if (!_gas_limitSet) {
